...
for o in np.arange(len(orders)):
    ...
    for m in np.arange(M):
        ...
        y_pred_test_permutation = clf.predict(X_test)
        # y_pred_test = np.loadtxt(os.path.join(model_dir, 'y_pred_test_rf.txt'), dtype='int')
        result_train = permutation_importance(clf, X_train_combined, y_train_combined,
                                              n_repeats=n_permutations, random_state=seed)
        permutation_importances_train = result_train.importances_mean
        result_test = permutation_importance(clf, X_test, y_test,
                                             n_repeats=n_permutations, random_state=seed)
        permutation_importances_test = result_test.importances_mean
        importances_mean = np.loadtxt(os.path.join(model_dir, 'importances_mean_rf.txt'))
        
        np.savetxt(os.path.join(model_dir, 'y_pred_test_rf_permutation.txt'), y_pred_test_permutation, fmt='%i')
        np.savetxt(os.path.join(model_dir, 'permutation_importances_train.txt'), permutation_importances_train)
        np.savetxt(os.path.join(model_dir, 'permutation_importances_test.txt'), permutation_importances_test)
